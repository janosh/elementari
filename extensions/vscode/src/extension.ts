...
const read_file = (file_path: string): FileData => {
...
const get_file = (uri?: vscode.Uri): FileData => {
  ...
  if (
    activeTab?.input && typeof activeTab.input === `object` &&
    activeTab.input !== null && `uri` in activeTab.input
  ) {
    return read_file((activeTab.input as { uri: vscode.Uri }).uri.fsPath)
  }
...
const handle_msg = async (msg: MessageData): Promise<void> => {
...
class Provider implements vscode.CustomReadonlyEditorProvider<vscode.CustomDocument> {
  ...
  resolveCustomEditor(
    ...
    try {
      ...
      webviewPanel.webview.html = create_html(
